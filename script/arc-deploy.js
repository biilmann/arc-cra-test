...
function areEqual(profile, secondProfile) {
  return (
    profile.accessKey === secondProfile.accessKey &&
    profile.secretAccessKey === secondProfile.secretAccessKey
  );
}

function replaceDefaultProfile(newDefaultProfileName) {
  console.log(
    `Replacing [default] profile creds with [${newDefaultProfileName}] creds`
  );
  ...
  if (!newDefaultProfile) {
    throw new Error(
      `No "${newDefaultProfileName}" profile found. Please check your creds`
    );
  }
  ...
  const newRawData = newDefaultProfile.rawText.replace(
    /^\s*\[([^[\]]+)]/,
    `[default]`
  );
  ...
  console.log(
    `Successfully replaced [default] profile with [${newDefaultProfileName}]`
  );
  return getProfiles();
}
